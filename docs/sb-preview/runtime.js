  ...
  "../../node_modules/qs/lib/stringify.js"(exports, module) {
    ...
    var getSideChannel = require_side_channel(),
      ...
      stringify2 = function stringify3(
        ...
        for (
          var adjustedPrefix = commaRoundTrip && isArray2(obj) && obj.length === 1 ? prefix + "[]" : prefix, j = 0;
          j < objKeys.length;
          ++j
        ) {
          var key2 = objKeys[j],
    ...
    module.exports = function (object, opts) {
      ...
      return (
        options2.charsetSentinel &&
          (options2.charset === "iso-8859-1" ? (prefix += "utf8=%26%2310003%3B&") : (prefix += "utf8=%E2%9C%93&")),
  ...
  Channel = class {
    constructor({ transport, async = !1 } = {}) {
      (this.sender = generateRandomId()),
        ...
        (this.isAsync = async),
        transport && ((this.transport = transport), this.transport.setHandler((event) => this.handleEvent(event)));
    }
    get hasTransport() {
  ...
  PostmsgTransport = class {
    ...
    getFrames(target) {
      if (this.config.page === "manager") {
        let list = Array.from(document.querySelectorAll("iframe[data-is-storybook][data-is-loaded]"))
          .filter((e) => {
            try {
    ...
    getCurrentFrames() {
      return this.config.page === "manager"
        ? Array.from(document.querySelectorAll('[data-is-storybook="true"]')).map((e) => e.contentWindow)
        : scope && scope.parent
        ? [scope.parent]
        : [];
    }
    getLocalFrame() {
      return this.config.page === "manager"
        ? Array.from(document.querySelectorAll("#storybook-preview-iframe")).map((e) => e.contentWindow)
        : scope && scope.parent
        ? [scope.parent]
        : [];
    }
...
var StoryRender = class {
    ...
    async render({ initial = !1, forceRemount = !1 } = {}) {
      ...
      try {
        let loadedContext;
        if (
          (await this.runPhase(abortSignal, "isLoading", async () => {
  ...
  Preview = class {
    ...
    initializeWithProjectAnnotations(projectAnnotations) {
      ...
      return storyIndexPromise
        ...
        .catch((err) => {
          throw (this.renderPreviewEntryError("Error isLoading story index:", err), err);
        });
    }
...
function executeLoadable(loadable) {
 ...
 ${error.stack}`
              : error.toString();
          logger.error(`Unexpected error while isLoading ${filename}: ${errorString}`);
        }
      });
    });
  else {
